...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.empty(len(w), self.n_features * self.embed_size)
        for i in range(len(w)):  # batch_size
            # each feature maps to an embedding > (numOfFeatures * embed_size)
            # But the output of index_select will be of size ( embed_size * numOfFeatures) so we need to reshape it.
            x[i] = torch.index_select(self.embeddings, 0, w[i]).view(-1)



        # Found this one line online and it's much faster than mine
    ...
    def forward(self, w):
        ...
        # Apply equations from PDF
        # h = ReLU(xW + b1)
        # l = hU + b2
        h = F.relu(torch.matmul(self.embedding_lookup(w), self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
