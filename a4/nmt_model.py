...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded)  # Initialized before in init function
        X_packed = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_packed)
        enc_hiddens, ll = pad_packed_sequence(enc_hiddens,
                                              batch_first=True)  # batch_first is true to transpose the output as needed.
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2))  # 2 for dimensions to be dec_hidden -> (b,h,1)
        e_t = e_t.squeeze(2)  # (b, src_len)
